...
class LobbyMenu:
    ...
    def draw_players(self, surface):
        for joueur in self.players:
            ...
            surface.blit(texte_nom, texte_nom.get_rect(center=(joueur.x + joueur.width // 2, joueur.y - 18)))

            texte_vies = self.font_info.render(f"HP: {joueur.vies}", True, (150, 255, 150))
            surface.blit(texte_vies, texte_vies.get_rect(center=(joueur.x + joueur.width // 2, joueur.y + joueur.height + 8)))
