...
class DeathScreen:
    ...
    def handle_input(self, event, mode_solo=False):
        if event.type == pygame.MOUSEBUTTONDOWN and event.button == 1:
            if self.btn_rejouer.collidepoint(event.pos): return "REJOUER"
            if not mode_solo and self.btn_spectateur.collidepoint(event.pos): return "SPECTATEUR"
        return None
    ...
    def draw(self, screen, mode_solo=False):
        ...
        pygame.draw.rect(screen, (150, 0, 0), self.btn_rejouer, border_radius=8)
        screen.blit(self.font_btn.render("REJOUER", True, (255, 255, 255)), (self.btn_rejouer.x + 35, self.btn_rejouer.y + 10))
        
        if not mode_solo:
            pygame.draw.rect(screen, (50, 50, 150), self.btn_spectateur, border_radius=8)
