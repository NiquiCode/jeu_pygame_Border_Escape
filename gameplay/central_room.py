...
class CentralRoom:
    ...
    def _get_player_rect(self, current_player_obj):
        return pygame.Rect(int(current_player_obj.x), int(current_player_obj.y), int(current_player_obj.width), int(current_player_obj.height))
    ...
    def roll_dice(self, total_players_in_game):
        ...
        while True:
            result = self.dice_manager.lancer_systeme_des(total_players_in_game, couleurs_portes)
            cible = result.get("salle_cible")
            
            if cible is not None:
                # --- CORRECTION DE COMPARAISON ULTRA-ROBUSTE ---
                if int(cible[0]) != int(self.map_manager.player_pos[0]) or int(cible[1]) != int(self.map_manager.player_pos[1]):
                    break
            else:
                break
        
        self.door_dice_results = result.get("portes", [])
        self.target_coords = cible
        
        # --- SÉCURITÉ PROTOCOLE SOLO ---
        if total_players_in_game == 1:
            self.required_players = 1
        else:
            self.required_players = result.get("joueurs_requis", 1)
    ...
    def apply_dice_result(self, data):
        ...
        if salle_cible is not None:
            ...
            self.target_name = ""
            self.dice_rolled = False
        self.required_players = data.get("joueurs_requis", data.get("required", 1))
        self.update_message()
    ...
    def _handle_table_interaction(self, player_rect, total_players_in_game, can_roll_dice):
        if not self._player_is_on_table(player_rect): return
    ...
    def _teleport_player_after_door(self, current_player_obj, direction):
        if direction == "HAUT": current_player_obj.y = self.height - current_player_obj.height - 40
        elif direction == "BAS": current_player_obj.y = 40
        elif direction == "GAUCHE":
            current_player_obj.x = self.width - current_player_obj.width - 40
            if hasattr(current_player_obj, "facing_right"): current_player_obj.facing_right = False
        elif direction == "DROITE":
            current_player_obj.x = 40
            if hasattr(current_player_obj, "facing_right"): current_player_obj.facing_right = True
    ...
    def _handle_target_room_arrival(self, current_pos, players_in_same_room):
        ...
        # --- CORRECTION DU DÉTECTEUR DE POSITION ---
        if int(current_pos[0]) == int(self.target_coords[0]) and int(current_pos[1]) == int(self.target_coords[1]):
            if players_in_same_room >= self.required_players: 
                return "LANCER_ENIGME"
            self.message = f"En attente... ({players_in_same_room}/{self.required_players} joueurs)"
            return "CENTRAL"
    ...
    def _handle_doors(self, current_player_obj):
        ...
        for direction, rect in self.doors.items():
            if direction not in available_doors: continue
            if not player_rect.colliderect(rect): continue

            if not self.dice_rolled:
                self.message = "ERREUR : Lancez les dés d'abord !"
                return "CENTRAL"
            
            if current_time - self.last_transition_time < self.transition_cooldown_ms: return "CENTRAL"
            
            current_player_obj.perdre_vie()
            moved = self.map_manager.move_player(direction)
            if not moved: return "CENTRAL"

            self.last_transition_time = current_time
            self._teleport_player_after_door(current_player_obj, direction)
            current_pos = self.map_manager.player_pos
            if current_pos == self.map_manager.exit_pos and self.map_manager.exit_revealed: return "FIN_DU_JEU"
            return "CENTRAL"
        return "CENTRAL"

    def update(self, current_player_obj, total_players_in_game=1, can_roll_dice=True):
        player_rect = self._get_player_rect(current_player_obj)
        self._handle_table_interaction(player_rect, total_players_in_game, can_roll_dice)
        door_result = self._handle_doors(current_player_obj)
        if door_result == "FIN_DU_JEU": return "FIN_DU_JEU"
        current_pos = self.map_manager.player_pos
        target_result = self._handle_target_room_arrival(current_pos, total_players_in_game)
        if target_result == "LANCER_ENIGME": return "LANCER_ENIGME"
        return "CENTRAL"

    def draw(self, screen):
        available = self.check_doors_availability()
        for direction in available: pygame.draw.rect(screen, (50, 50, 50), self.doors[direction])
        ...
        if self.dice_rolled and self.door_dice_results:
            ...
            for resultat in self.door_dice_results:
                ...
                txt = self.font_small.render(f"{couleur} : dé={resultat_de} | cap={capacite}", True, (255, 255, 255))
                screen.blit(txt, (x, y))
                y += 24