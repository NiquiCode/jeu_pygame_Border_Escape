        ...
        if self.dice_rolled and self.door_dice_results:
            for porte in self.door_dice_results:
                ...
                if rect_porte and player.rect.colliderect(rect_porte):
                    ...
                    if pos in ("haut", "n", "up", "north"): self.map_manager.player_pos[1] -= 1; player.y = self.height - player.size - 40
                    elif pos in ("bas", "s", "down", "south"): self.map_manager.player_pos[1] += 1; player.y = 40
                    elif pos in ("gauche", "o", "w", "left", "west"): self.map_manager.player_pos[0] -= 1; player.x = self.width - player.size - 40
                    elif pos in ("droite", "e", "right", "east"): self.map_manager.player_pos[0] += 1; player.x = 40
